---
description: Python best practices for type annotations, data structures, error handling, paths, async, naming, imports, and docstrings.
paths:
  - '**/*.py'
  - '**/*.py*.jinja'
---
...
Borrowed from: https://github.com/cuioss/plan-marshall/blob/main/marketplace/bundles/pm-dev-python/skills/python-best-practices/standards/python-core.md

---

## Type Annotations

The goal is complete type annotations for all code, and to use `ty` to check types. This helps catch bugs early and makes it easier to understand how to use functions and classes.

If typing a function is difficult, it's often a sign that the function is doing too much and should be refactored into smaller functions. In that case, prioritize refactoring and adding types to the new smaller functions, rather than trying to add types to the original large function.
...
Generally, we don't want to ignore type checking, but sometimes it's not worth the effort to type something correctly or it just may be impossible due to Python or `ty`. In those cases, use `# ty:ignore[specific-issue]` comments to ignore specific lines and issues. Examples:

```python
# TODO: Add a comment on why this is being ignored
some_var: int = "string"  # ty:ignore[assignment]
```

VS Code has a nice "Quick Fix" option that you can choose when hovering over the warning that will add the correct ignore comment for you.

Note that a more generic `# type: ignore[example]` can be used that is not `ty` specific, but `ty` doesn't seem to respect the specific thing to ignore, so this effectively ignores all type checking on the line.
