...
<script>
  ...
  function getSentryData() {
    ...
    Sentry.startSpan(
      ...
      async () => {
        console.log('here');
        const res = await fetch('/sentry-example-not-found');
        if (!res.ok) {
...
<div>
  <main>
    ...
    <button type="button" on:click={console.log}> Throw error! </button>

    <p>
      2. Look for the error on the
      <a href="https://protect-democracy.sentry.io/issues/?project=4506938812792832">Issues Page</a
      >.
    </p>
